...
ImuSerialNode::ImuSerialNode():Node("imu_serial")
{
    ...
    this->__timer__ = this->create_wall_timer(0.01s,std::bind(&ImuSerialNode::timer_callback,this));
}
            ...
            switch(this->stage)
            {
                ...
                case STAGE_READING_PACKET:
                {
                    ...
                    if(checksum(this->rx_buffer,10))
                    {
                        ...
                        if(this->cmd_id == 0x51)
                        {
                            acc_x = hex_to_short(this->rx_buffer[3],this->rx_buffer[2]) / 32768.0 * 16 * 9.8 / 32768.0 * 16;
                            acc_y = hex_to_short(this->rx_buffer[5],this->rx_buffer[4]) / 32768.0 * 16 * 9.8 / 32768.0 * 16;
                            acc_z = hex_to_short(this->rx_buffer[7],this->rx_buffer[6]) / 32768.0 * 16 * 9.8 / 32768.0 * 16;
                        }
                        else if(this->cmd_id == 0x52)
                        {
                            gym_x = hex_to_short(this->rx_buffer[3],this->rx_buffer[2]) / 32768.0 * 2000 * M_PI / 180 / 32768.0 * 2000;
                            gym_y = hex_to_short(this->rx_buffer[5],this->rx_buffer[4]) / 32768.0 * 2000 * M_PI / 180 / 32768.0 * 2000;
                            gym_z = hex_to_short(this->rx_buffer[7],this->rx_buffer[6]) / 32768.0 * 2000 * M_PI / 180 / 32768.0 * 2000;
                        }
